...
class BNNPipeline():
    """ A class to handle the BNN pipeline for a specific model type.
    It manages the loading of weights, populating tables, and managing the input registers.
    TESTED ONLY ON TINY AND DENSE MODELS.
    """
    def __init__(self, bnn_model, bfrt, input_length):
        ...
        self.bfrt_confidence_lookup = self.p4_bnn_exec.BnnIngress.confidence_lookup

        self.w1 = None
        self.w2 = None
        if(self.bnn_model == 'wide'):
            self.w3 = None
            self.parallel_neurons_cap = 4
            self.parallel_bit_cap = 16
        ...
        elif(self.bnn_model == 'dense' or self.bnn_model == 'tiny'):
            self.parallel_neurons_cap = 7
            self.parallel_bit_cap = 14
            ...
            raise ValueError(f"Unknown BNN model: {self.bnn_model}")

        if self.input_length % self.parallel_bit_cap != 0:
            self.input_length = (self.input_length // self.parallel_bit_cap + 1) * self.parallel_bit_cap
            print(f"Adjusted input length to {self.input_length} to be multiple of {self.parallel_bit_cap}")
    ...
    def load_weights_last_layer(self, l2_w):
        ...
        num_neuron_batches3 = nn[2] // 2
        weight_batches_no3  = len(l2_w[0]) // self.parallel_bit_cap
        assert len(l2_w[0]) % self.parallel_bit_cap == 0
        ...
        for ix, w in enumerate(l2_w):
            ...
            chunks = [ w[i:i+self.parallel_bit_cap] for i in range(0, len(w), self.parallel_bit_cap) ]
        ...
        for j in range(num_neuron_batches3):
            for ix in range(weight_batches_no3):
                ...
                row = w_mx2[ix]
                w0, w1 = row[base: base + self.parallel_neurons_cap]
    ...
    def load_weights_tb_7(self, l1_w, l2_w):
        l1_w = [f'{int(val, 16):0{self.input_length}b}' for val in l1_w]
        l2_w = ['{:042b}'.format(int(l2_w[i], 16)) for i in range(0, len(l2_w))]

        assert len(l1_w[0]) % self.parallel_bit_cap == 0
        ...
        num_neuron_batches  = nn[1] // self.parallel_neurons_cap
        weight_batches_no = len(l1_w[0]) // self.parallel_bit_cap

        assert len(l1_w) == self.parallel_neurons_cap * num_neuron_batches, "hex_w length must equal num_weight_batches * num_neuron_batches"

        w_mx = np.zeros((weight_batches_no, len(l1_w))).tolist()
        for ix, w in enumerate(l1_w):
            chunks = [w[i:i+self.parallel_bit_cap] for i in range(0, len(w), 14)]
            for jx, chunk in enumerate(chunks):
                w_mx[jx][ix] = chunk
        ...
        for j in range(num_neuron_batches):
            for ix in range(weight_batches_no):
                ...
                (w0,  w1,  w2,  w3, w4, w5, w6) = w1[base: base+self.parallel_neurons_cap]
        ...
        print("→ Done loading all 133-bit weights into l0_weights (two-key table).\n")

        self.load_weights_last_layer(l2_w)
    ...
    def loead_weights_tb_4(self, l1_w, l2_w, l3_w):
        ...
        assert len(l1_w) == self.parallel_neurons_cap * num_neuron_batches, \
        ...
        for j in range(num_neuron_batches):
            for ix in range(weight_batches_no):
                ...
                (w0,  w1,  w2,  w3) = w1[base: base+self.parallel_neurons_cap]
        ...
        num_neuron_batches  = nn[2] // self.parallel_neurons_cap #2
        weight_batches_no = len(l2_w[0]) // self.parallel_bit_cap #8

        assert len(l2_w) == self.parallel_neurons_cap * num_neuron_batches, \
    ...
    def load_bnn_input_reg_7(self, bit_input):
        assert len(bit_input) == self.input_length, f"hex_input must be exactly {self.input_length} bin digits"

        old_offset = self.bnn_input_offset

        input_chunks = [ f'00{bit_input[i : i + 14]}' for i in range(0, self.input_length, 14) ]
        ...
        print(f"→ Done loading the {self.input_length}-bit inputs into registers.\n")

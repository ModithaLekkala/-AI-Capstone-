...
control Bytes(inout collector_headers_t hdr, inout metadata_t meta) {
    ...
    /* Max bytes register actions */
    RegisterAction<_, bit<16>, bit<16>>(flows_smaxbytes) update_flows_smaxbytes = {
        void apply(inout bit<16> max_bytes, out bit<16> rv) {
            if (meta.frame_len > max_bytes) {
                max_bytes = meta.frame_len;
            }
            rv = max_bytes;
        }
    };
    ...
    /* Min bytes register actions - initialize with max value */
    RegisterAction<_, bit<16>, bit<16>>(flows_sminbytes) update_flows_sminbytes = {
        void apply(inout bit<16> min_bytes, out bit<16> rv) {
            if (min_bytes == 0 || meta.frame_len < min_bytes) {
                min_bytes = meta.frame_len;
            }
            rv = min_bytes;
        }
    };
